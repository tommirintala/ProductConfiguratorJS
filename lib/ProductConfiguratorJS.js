...
ProductConfiguratorJS.prototype.appendElementHtml = function(node) {
	...
	case 'boolean':
	    ...
	    template_end = "></label>";
	    template += '<label>' + node.label+ '<input type="checkbox" name="' + node.id
		+ '" id="' + node.id + '"';
	    // add dynamic rules handling
	    break;
	case 'list':
	    //e = $("input[type='" + node.type + "']");
	    e = jQuery("select");
	    //template_end = "></select></label>";
	    //template = '<label>' + node.label + '<select id="' + node.id;
	    if ( typeof node.values !== 'undefined' ) {
		//var o = [];
		for (var i=0;i<node.values.length;i++) {
		    // template = template + '<option value="' + node.values[i] + '">' + node.values[i] + '</option>';
		    $("<option/>", {value: node.values[i], text: node.values[i]}).appendTo(e); // o.push('<option value="' + node.values[i] + '">' + node.values[i] + '</option>');
		}
		// e.html(o.join("\n"));
	    }
	    // add dynamic rules handling
	...
	case 'text':
		...
		if ( node.hasOwnProperty(keywords[i]) ) {
		    ...
		    e.attr(keywords[i], node[keywords[i]]);
		}
	    }
	    // template_end = '></label>';
	    break;
	...
	if ( node.enabled ) {
	    ...
	    e.attr('enabled', 'false');		   
	}

	// console.log( e. );
	
	//this._form = this._form + row_begin + e.html() + row_end;
	//return row_begin + e.html() + row_end;
	// e.appendTo("#" + this._element);
	// var s = jQuery(this._element);
	jQuery(this._element).append(e.html()); // .html( e );
    }
};

ProductConfiguratorJS.prototype.createForm = function() {
    //this._form = ""; // <table>";
    jQuery(this._element).html('');
